...
@Component
public class JWTAuthenricationHttp403 extends Http403ForbiddenEntryPoint {

@Override
public void commence(HttpServletRequest request, HttpServletResponse response, AuthenticationException arg2) throws IOException {
 MicroServiceResponse microServiceResponse=new MicroServiceResponse(HttpStatus.FORBIDDEN.value()
         ,HttpStatus.FORBIDDEN
         ,HttpStatus.FORBIDDEN.getReasonPhrase().toUpperCase(),
         "You Need to login to access this resource");
 microServiceResponse.setHttpStatus(HttpStatus.FORBIDDEN);
